	...
	<head>
		...
		<script src="//t1.daumcdn.net/mapjsapi/bundle/postcode/prod/postcode.v2.js"></script>
			...
			<form action="<c:url value='/signup'></c:url>" method="POST" class="form" id="signup_frm">
				...
				<div class="alert_pw"> 비밀번호는 8~16자, 영문+숫자+!/@/#/$만 가능합니다.</div>
				<input type="password" class="pw2" name="me_pw2" placeholder="비밀번호 확인">
				<input type="email" class="em" name="me_ma_email" placeholder="이메일">
				<button type="button" id="email_auth_btn" class="btn-emails">이메일 인증 코드 보내기</button>
					
				<br>
				<br>
				<input type="text" name="auth" id="email_auth_key" placeholder="인증 코드 입력" style="display:none;">
				<button type="button" class="btn-email">이메일 인증</button>
				...
				<input class="form-control" style="width: 40%; display: inline;" placeholder="우편번호(선택)" name="me_post" id="me_post" type="int" readonly="readonly" >
   					 <button type="button" class="btn btn-default" onclick="execPostCode();"><i class="fa fa-search"></i> 우편번호 찾기</button>                               
				</div>
				<div class="form-group">
    			<input class="form-control" style="top: 5px;" placeholder="도로명 주소(선택)" name="me_street_address" id="me_street_address" type="text" readonly="readonly" />
				</div>
				<div class="form-group">
   				 <input class="form-control" placeholder="상세주소(선택)" name="me_detail_address" id="me_detail_address" type="text"  />
				</div>
...
$('[name=me_id]').keyup(function() {
	...
	if (!regex.test(me_id)) {
		...
		$(".iderr").text('아이디는 5~13자(영문+숫자 or 영문)만 가능합니다.');
	   	return;
	...
	$.ajax({
		...
		success : function(data){
		    if(data.res){
		    	$(".iderr").text('사용 가능한 아이디입니다.');
		    	idCheck = true;
		    	$('[name=me_id]').css('background', 'lightgreen');
		    }else{
		    	$(".iderr").text('이미 사용중인 아이디입니다.')
